...
function setup(){
  ...
  document.getElementById("dev").appendChild(cmd);
      document.getElementById("cmd").spellcheck=false;
...
function colorConsole(msg, color,ligne) {
 ...
 consoleContent=msg;
    ...
    function createElement(what,where,ptX,ptY,posX,posY,posZ,posW,posH,html,style){
      ...
      arrW.push(posW+100);
      arrH.push(posH+100);
      TempArrX.push(posX-100);
     TempArrY.push(posY-100);
     
     TempArrW.push(posW+100);
     TempArrH.push(posH+100);
      }else{
      arrX.push(posX);
      arrY.push(posY);
...
style=String(style);
html = html.replace(style,"style=\"width:100%;\"");
