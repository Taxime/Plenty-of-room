//  v161110 : atelier thales "
// format url http://localhost/plenty_of_room/index161110#<dataName>coucou</dataName><x>54</x><y>128</y><z>13</z>
    ...
    camProp={x:0,y:0,z:1,haut:false,bas:false,droite:false,gauche:false,
     ...
     function initByUrl(){
      ...
      if (url.includes("<x>") && url.includes("</x>")) {
       ...
       camProp.x=parseFloat(initX);
     }
     if (url.includes("<y>") && url.includes("</y>")) {
       var initY = url.substring(url.indexOf("<y>") + 3, url.indexOf("</y>"));
       camProp.y=parseFloat(initY);
     }
     if (url.includes("<z>") && url.includes("</z>")) {
      var initZ = url.substring(url.indexOf("<z>") + 3, url.indexOf("</z>"));
      camProp.z=parseFloat(initZ);
    }
    if (url.includes("<dataName>") && url.includes("</dataName>")) {
      var initDataName = url.substring(url.indexOf("<dataName>") + 10, url.indexOf("</dataName>"));
...
if(install==true){
  ...
  if(a>20){
    if(isMouseDown==false){
      load(dataName);
      if(cameraIsNotMoving==true && changingUrl==false && mouse.move==false && mouse.down==false){
        //console.log("camProp.x"+camProp.x);
        //console.log("camProp.y"+camProp.y);
        //console.log("camProp.z"+camProp.z); 
        ChangeUrl("Plenty Of Room"+String(dataName),"#<dataName>"+dataName+"</dataName><x>"+camProp.x+"</x><y>"+camProp.y+"</y><z>"+camProp.z+"</z>");
        changingUrl=true;
      }
    }
    a=0;
...
if(Elements[i].getAttribute("type")=="nope" && TempArrZ[i]>8){
 ...
 if(TempArrZ[i]<W/2){
  if(Elements[i].getAttribute("type")=="nope"){
...
function echelleValue(cm) {//µm
  var values=[1];
  var value="<br>" + 2000;
  var left;
  for (var i = 1; i < 100; i++) {
   values.push(values[values.length-1]*1.033);
 }
 for (var i = 1; i < 99; i++) {
  if(cm>1*values[i]&&cm<1*values[i+1]){
    value="<br>"+(2001-i);
    left=i*10;
  }
}
...
function echelle(){
  document.getElementById("echelle").innerHTML="<div style=\"margin-left:"+echelleValue(camProp.z).left+"px;\">"+echelleValue(camProp.z).date+"</div>";
}
...
if(toPrint==true){
  ...
  //_______________________________________________________
    ...
    //_______________________________________________________
    // 
    var changingUrl=true;
    var isInstallDone=false;
    var lastLoop = new Date;
    var refreshing=10;
    var isMouseDown;
    var mouse={x:undefined,y:undefined,xDown:undefined,yDown:undefined,down:false,move:false};
    var update='';
...
function teleport(x,y,z){
  ...
  camProp.x=x;
  camProp.y=y;
  refreshPositions(999999);
  //console.log("teleportOK");
...
if(str[0]=='!'){
  ...
  if (newStr.includes(".youtube.")) {
   ...
   document.getElementById("preview").style.width=560+"px";
   document.getElementById("preview").style.height=315+"px";
 }
}else{
...
////////////////////////////////////
    ...
    document.onmousemove = function(event) { 
      if(mouse.x!=event.clientX && mouse.x!=event.clientX ){
        mouse.move=true;
      }else{
        mouse.move=false;
      }
      ...
      if(mouse.down==true)
      {/*
        var newPosX=camProp.x-(mouse.xDown-mouse.x)/camProp.z;
        var newPosY=camProp.y-(mouse.yDown-mouse.y)/camProp.z;
        camProp.x=newPosX;
        camProp.y=newPosY;
        refreshPositions(256);//
        */
...
document.onmousedown=function(event){
  if(event.button==2){
    ...
    rightClick=false;
    mouse.down=true;
    mouse.xDown=event.clientX;
    mouse.yDown=event.clientY;
  }
  isMouseDown = true;
  if(document.getElementById("cmd").value==""||document.getElementById("cmd").value==" "){
    if(rightClick==true){
  ...
  var str=document.getElementById("cmd").value;
  if (str.includes("<lien>") && str.includes("</lien>")) {
    if(str.includes("<color>") && str.includes("</color>") ){
      var liencolor= str.substring(str.indexOf("<color>") + 7, str.indexOf("</color>"));
    }else{
      var liencolor="blue";
    }
    if(str.includes("<fontcolor>") && str.includes("</fontcolor>") ){
      var lienfontcolor= str.substring(str.indexOf("<fontcolor>") + 11, str.indexOf("</fontcolor>"));
    }else{
      var lienfontcolor="white";
    }
    if(str.includes("<weight>") && str.includes("</weight>") ){
      var lienweight= parseInt(str.substring(str.indexOf("<weight>") + 8, str.indexOf("</weight>")));
    }else{
      var lienweight=1;
    }
    if(str.includes("<name>") && str.includes("</name>") ){
      var lienname = str.substring(str.indexOf("<name>") + 6, str.indexOf("</name>"));
    }else{
      var lienname='';
    }
    if(str.includes("<meta>") && str.includes("</meta>") ){
      var lienmeta = str.substring(str.indexOf("<meta>") + 6, str.indexOf("</meta>"));
    }else{
      var lienmeta=lienname;
    }
    if(lienmeta==''){
      lienmeta=lienname;
    }
    if(mousePosX.length==2){
      var x1 = parseFloat(str.substring(str.indexOf("<x1>") + 4, str.indexOf("</x1>")));
      var x2 = parseFloat(str.substring(str.indexOf("<x2>") + 4, str.indexOf("</x2>")));
      var y1 = parseFloat(str.substring(str.indexOf("<y1>") + 4, str.indexOf("</y1>")));
      var y2 = parseFloat(str.substring(str.indexOf("<y2>") + 4, str.indexOf("</y2>")));
      document.getElementById("cmd").value=linking(x1,x2,y1,y2,liencolor,lienfontcolor,lienweight,lienname,lienmeta);
      mousePosX=[];
      mousePosY=[];
      displayPreview();
      //console.log("mousePosX.length==2",document.getElementById("cmd").value);
    }else{
      if(mousePosX.length==1){
        document.getElementById("cmd").value="<lien><name>"+lienname+"</name><meta>"+lienmeta+"</meta><color>"+liencolor+"</color><fontcolor>"+lienfontcolor+"</fontcolor><weight>"+lienweight+"</weight><x1>"+mousePosX[mousePosX.length-1]+"</x1><y1>"+mousePosY[mousePosY.length-1]+"</y1><x2>"+String(mouse.x)+"</x2><y2>"+String(mouse.y)+"</y2></lien>";
        mousePosX.push(mouse.x);
        mousePosY.push(mouse.y);
        //console.log(document.getElementById("cmd").value);
      }
      if(mousePosX.length<1){
       document.getElementById("cmd").value="<lien><name>"+lienname+"</name><meta>"+lienmeta+"</meta><color>"+liencolor+"</color><fontcolor>"+lienfontcolor+"</fontcolor><weight>"+lienweight+"</weight><x1>"+String(mouse.x)+"</x1><y1>"+String(mouse.y)+"</y1></lien>";
       mousePosX.push(mouse.x);
       mousePosY.push(mouse.y);
       //console.log("mousePosX.length<1",document.getElementById("cmd").value);
     }
   }
...
var mousePosX=[];
var mousePosY=[];
function linking( x2,x1,y2,y1, color,fontcolor, thickness, tag,meta) {
    // distance
    ...
    var lineCss ="text-align:center;padding:0px; margin:0px; border-bottom:solid; border-width:" + thickness + "px; border-color:" + color + "; height:0px;line-height:0px;width:" + length + "px; -moz-transform:rotate(" + angle + "deg); -webkit-transform:rotate(" + angle + "deg); -o-transform:rotate(" + angle + "deg); -ms-transform:rotate(" + angle + "deg); transform:rotate(" + angle + "deg)";
    var lineHtml="<div style=\""+lineCss+"\"><font name=\""+tag+"\" meta=\""+meta+"\" onmouseout=\"tagout(this)\" onmouseover=\"tagover(this)\"style=\"background-color:"+color+";color:"+fontcolor+";font-size:12pt;\">"+tag+"</font></div>";
    return lineHtml;
  }
  function tagover(e){
    e.innerHTML=e.getAttribute("meta");
  }
  function tagout(e){
    e.innerHTML=e.getAttribute("name");
  }
