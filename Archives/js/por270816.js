  //  v270816 = d3.js + Vladimir Agafonkin simplify.js sketchpad
  //_______________________________________________________
    //
    //                 fonctions anonymes
    //_______________________________________________________
    // 
    function updatePaths(what,data) {
...
var pathUpdate=[];
for (var j = 0; j < paths[toDisplay[i]].path.length; j++) {
...
var cible = document.getElementsByClassName("line")[toDisplay[i]];
updatePaths(cible,pathUpdate);
}
...
function drawing(){
  ...
  function setupSvg(){
    ...
    for (var i = 0; i < simplyPath.length; i++) {
      xPathToSend.push(simplyPath[i].x);
      yPathToSend.push(simplyPath[i].y);
    }
    path=[];
