//  v160909 : pouvoir zoomer sur le texte , ameliorer le framerate (instance d'exploration / observation), chargement objets rouge, préciser l'échelle, correction du bug des photos 
/*
...
if(CameraStopping>5){
  if(cameraIsNotMoving!=true){
    cameraIsNotMoving=true;
    console.log("cameraIsNotMoving");
    refreshPositions(999999999);
  }
}else{
...
.attr("stroke", "rgb(72,72,72)")
  ...
  function refreshPositions(lagLimit){
    console.log("refreshing : "+lagLimit);
...
document.getElementById("composition").innerHTML="Elements<br>visible : "+toDisplay.length+"<br>total : "+Elements.length;
...
function echelleValue(cm,mm,micron) {//µm
  var value="<br>";
  if(cm>0&&cm<1000){
...
function echelle(){
  ...
  var mm=arrondir(10/camProp.z,1);
  var micron=parseInt(10000/camProp.z);
  document.getElementById("echelle").innerHTML=echelleValue(cm,mm,micron);
}
